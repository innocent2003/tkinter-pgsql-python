...
def validate_vn_phone(phone):
    vn_phone_pattern = r"^(03|05|07|08|09|02)\d{8,9}$"
    return re.match(vn_phone_pattern, phone)
...
def create_record():
    ...
    if not validate_vn_phone(phone):
        messagebox.showerror("Validation Error", "Invalid Vietnamese phone number!")
        return
